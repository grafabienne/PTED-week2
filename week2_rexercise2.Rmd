...
caro_3_2<-as.data.frame(rollmean(caro$speed, k =3, fill= NA, align = "left"))
caro_3_2$interval<-"3"
colnames(caro_3_2)<-c("Speed","Window")

caro_6_2<-as.data.frame(rollmean(caro$speed, k =6, fill = NA, align = "left"))
caro_6_2$interval<-"6"
colnames(caro_6_2)<-c("Speed","Window")


caro_9_2<-as.data.frame(rollmean(caro$speed, k =9, fill = NA, align = "left"))
caro_9_2$interval<-"9"
colnames(caro_9_2)<-c("Speed","Window")
caro_2<-rbind.data.frame(caro_3_2,caro_6_2,caro_9_2)

ggplot(caro_2, aes(Speed, col=Window))+
  geom_freqpoly()+
  labs(x="Speed [m/s]")
...
```
The first plot shows, with smaller moving windows (k), the flatter the curve. More different values for speed are shown. Very high or very low values are better represented with a small moving window, as shown in the second graph (boxplot).
